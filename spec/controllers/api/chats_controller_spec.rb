    ...
    context "for a visitor" do
      describe "#create" do
        ...
        it "should create a new chat" do
          ...
          allow(Pusher).to receive(:[]).with("operator_#{operator.operator_uid}") { mock_client }
        ...
        it "should use the operator with the least number of open chats if the operator requested is full" do
          ...
          mock_client = double('client')
          allow(Pusher).to receive(:[]).with("operator_#{operator3.operator_uid}") { mock_client }
          expect(mock_client).to receive(:trigger)
    ...
    context "for an operator" do
      ...
      describe "#collect_stats" do
        it "should work" do
          ...
          expect(Rest).to receive(:post).with("https://www.missionhub.com/apis/v3/people?secret=missionhub_token&permissions=2&person[first_name]=#{chat.visitor.first_name}&person[email]=new@email.com").and_return("person" => {"id" => mh_id})
          expect(Rest).to receive(:post).with("https://www.missionhub.com/apis/v3/contact_assignments?secret=missionhub_token&contact_assignment[person_id]=#{mh_id}&contact_assignment[assigned_to_id]=#{chat.operator.missionhub_id}")
          expect(Rest).to receive(:post).with("https://www.missionhub.com/apis/v3/contact_assignments?secret=missionhub_token&contact_assignment[person_id]=#{mh_id}&contact_assignment[assigned_to_id]=#{chat.operator_whose_link.missionhub_id}")
          expect(Rest).to receive(:post).with("https://www.missionhub.com/apis/v3/followup_comments", {:secret=>chat.campaign.missionhub_token, :followup_comment => { :contact_id => chat.visitor.missionhub_id, :commenter_id => chat.operator.missionhub_id, :comment => chat.build_comment(params) }}).and_return("followup_comment" => [{"id" => 1}])
        ...
        it "should handle wonky emails that get passed in by the client every so often" do
          ...
          expect(Rest).to receive(:post).with("https://www.missionhub.com/apis/v3/people?secret=missionhub_token&permissions=2&person[first_name]=#{chat.visitor.first_name}&person[email]=#{chat.visitor.email}").and_return("person" => {"id" => mh_id})
          expect(Rest).to receive(:post).with("https://www.missionhub.com/apis/v3/contact_assignments?secret=missionhub_token&contact_assignment[person_id]=#{mh_id}&contact_assignment[assigned_to_id]=#{chat.operator.missionhub_id}")
          expect(Rest).to receive(:post).with("https://www.missionhub.com/apis/v3/contact_assignments?secret=missionhub_token&contact_assignment[person_id]=#{mh_id}&contact_assignment[assigned_to_id]=#{chat.operator_whose_link.missionhub_id}")
          expect(Rest).to receive(:post).with("https://www.missionhub.com/apis/v3/followup_comments", {:secret=>chat.campaign.missionhub_token, :followup_comment => { :contact_id => chat.visitor.missionhub_id, :commenter_id => chat.operator.missionhub_id, :comment => chat.build_comment(params) }}).and_return("followup_comment" => [{"id" => 1}])
          post :collect_stats, params

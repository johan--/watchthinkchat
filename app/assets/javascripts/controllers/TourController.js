angular.module('chatApp').controller('TourController', ['$scope', function($scope) {
  ...
  bt.addSteps([
    ...
    },
    {
      ...
      content: "<p class='lead'>The first step is to inform visitors that chat is ready.</p><p>See our bot buddy down the bottom? We've used a persona to invite visitors to chat.</p><a href='#' data-ng-click='showNextStep();'><div class='alert alert-info text-center'>Pause the video</div></a>",
      //next: -1,
      onNext: function(bt) {
        player.pauseVideo();
      },
    ...
    },
    {
      ...
      backdrop: true,
      prev: 6,
      content: "<p class='lead'>That's all there is to it. Thanks for checking out Godchat. </p><p>Godchat is all about allowing crowd sourcing of missionaries. Because everyone is a missionary.<br><br><a class=\"btn-lg btn-success btn-block text-center\" href=\"#\">Sign up for free</a><p>",
      next: -1
    }
  ]);
  ...
  var onPlayerStateChange = function(evt){
    switch(evt.data) {
      case YT.PlayerState.PLAYING:
        ...
        if (parseInt(bt.getState("current_step")) > 1)
          bt.hideStep( bt.getState("current_step") );
        hideChat();
        break;
      ...
      case YT.PlayerState.PAUSED:
        ...
        if (parseInt(bt.getState("current_step")) > 1)
          bt.showStep( bt.getState("current_step") );
        showChat();
        break;
    }
  };

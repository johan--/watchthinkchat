ActiveAdmin.register Chat do
  ...
  filter :operator,
         collection: (proc do
           User.operators.order('first_name asc, last_name asc')
         end)
  filter :operator_whose_link,
         collection: proc { User.operators },
         label: 'Link Sharer'
  ...
  index do
    ...
    column 'Header' do |chat|
      html = ''.html_safe
      chat.messages.first(8).map(&:body).each do |body|
        html << body
        html << '<br>'.html_safe
      end
      html
  ...
  show do
    panel 'Details' do
      attributes_table_for chat do
        [:operator, :visitor, :created_at, :updated_at,
         :campaign, :status, :uid].each do |column|
          row column
        end
      end
    end
  ...
  controller do
    def scoped_collection
      ...
      (params[:q] || {}).each do |scope, value|
        next unless Chat.respond_to?(scope)

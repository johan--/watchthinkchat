class Api::CampaignsController < ApplicationController
  ...
  def update
    ...
    unless @campaign.admin?(current_user)
  ...
  def create
    @campaign = Campaigns::AsCreate.new
    create_or_update_campaign
  end
  ...
  def password
    ...
    if params[:password].present? &&
       @campaign.try(:password) == params[:password]
  ...
  def stats
    ...
    rows = @campaign.operators.map do |o|
      live_chats = o.operator_chats.where(campaign: @campaign,
                                          status: 'open').map(&:uid)
      # count chats
  ...
  def create_or_update_campaign
    ...
    campaign_params = params.permit(:uid,
                                    :followup_buttons,
                                    :name,
                                    :cname,
                                    :created_at,
    ...
    if params[:followup_buttons].present?
      ...
      params.require(:followup_buttons).each_with_index do |fb, i|
        ...
        errors += new_button.errors.to_a.map do |err|
          "Followup button #{i + 1}: #{err}"
        end
      end
    ...
    if @campaign.valid?
      # At this point, the campaign update is valid and
      # all new followup buttons are valid.
      # Delete all old buttons and make new ones.
      @campaign.followup_buttons.delete_all
      if new_buttons.present?
        new_buttons.map do |b|
          b.campaign = @campaign
          b.save!
        end
      end

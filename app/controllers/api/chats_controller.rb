class Api::ChatsController < ApplicationController
  ...
  def show
    ...
    if chat
      render json: {
        ...
        visitor: { uid: chat.visitor.visitor_uid,
                   name: chat.visitor.name },
        operator: { uid: chat.operator.operator_uid,
                    name: chat.operator.name },
        transcript: chat.messages
      }
    else
      render json: { error: 'chat_not_found',
                     message: 'Sorry, that chat can not be found' },
             status: 403
    end
  end
  ...
  def create
    ...
    if campaign && campaign.opened? && visitor && operator && operator.online?
      chat = Chat.create!(campaign_id: campaign.id,
      ...
      render json: { error: 'campaign_closed',
                     message: 'Sorry, the campaign was closed' },
             status: 403
    elsif !operator
      render json: { error: 'no_operators_available',
                     message: 'Sorry, no operators are available' },
             status: 500
    elsif !campaign
      render json: { error: 'campaign_not_found',
                     message: 'Campaign not found' },
             status: 404
    elsif !visitor
      render json: { error: 'visitor_not_found',
  ...
  def collect_stats
    chat = Chat.where(uid: params[:uid]).first
    if chat
      chat.collect_stats(params.slice(:visitor_response,
                                      :visitor_name,
                                      :visitor_email,
                                      :calltoaction,
                                      :notes))
  ...
  def ensure_operator
    return if signed_in? && current_user.operator
